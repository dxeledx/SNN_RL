...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Supervised pretrain (distillation): match per-subject LDA posteriors for commit-left/right logits"
    )
...
def _split_train_val(
    *, X: np.ndarray, y: np.ndarray, probs: np.ndarray, val_frac: float, seed: int
    ...
    return X[tr_idx], y[tr_idx], probs[tr_idx], X[val_idx], y[val_idx], probs[val_idx]
...
def _fit_lda_probs_per_subject(*, items: dict[int, SubjectWindows]) -> dict[int, np.ndarray]:
    ...
    for subj, sw in items.items():
        ...
        n_trials, n_steps, obs_dim = X.shape
...
def main() -> None:
    ...
    probs_all = np.concatenate(all_p, axis=0)

    X_train, y_train, P_train, X_val, y_val, P_val = _split_train_val(
        X=X_all, y=y_all, probs=probs_all, val_frac=val_frac, seed=seed
    )
    ...
    logger = CSVLogger(out_dir / "metrics_distill.csv")
    best_val_acc = -1.0
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, int(perm.shape[0]), batch_trials):
            ...
            pb = torch.as_tensor(P_train[idx], device=device, dtype=torch.float32)  # [B,S,2]

            ac.reset_all_states()
            loss = torch.tensor(0.0, device=device)
            for t in range(S):
                logits, _value, _sr = ac(xb[:, t])
                commit_logits = logits[:, 1:3]  # commit-left/right
                logp = torch.log_softmax(commit_logits, dim=-1)
                loss = loss + -(pb[:, t] * logp).sum(dim=-1).mean()
            loss = loss / float(S)
        ...
        with torch.no_grad():
            ...
            for start in range(0, int(X_val.shape[0]), batch_trials):
                ...
                pb = torch.as_tensor(P_val[start : start + batch_trials], device=device, dtype=torch.float32)
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(ac.state_dict(), str(run_paths.ckpt_dir / "distill_best.pt"))

    torch.save(ac.state_dict(), str(run_paths.ckpt_dir / "distill_last.pt"))
    logger.close()
    print(f"[distill] done -> {run_paths.out_dir}")
