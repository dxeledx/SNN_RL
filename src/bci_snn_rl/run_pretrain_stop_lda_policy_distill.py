...
from bci_snn_rl.data.io import SubjectWindows, group_by_subject, load_subject_windows
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Supervised pretrain (policy distill): distill a 3-action STOP policy from per-subject LDA posteriors"
    )
...
def _split_train_val(
    *, X: np.ndarray, y: np.ndarray, pi: np.ndarray, val_frac: float, seed: int
    ...
    return X[tr_idx], y[tr_idx], pi[tr_idx], X[val_idx], y[val_idx], pi[val_idx]


def _fit_lda_probs_per_subject(*, items: dict[int, SubjectWindows]) -> dict[int, np.ndarray]:
    """
    Fit per-subject LDA (window-level) and return teacher probs on the same subject's train windows.
    Returns:
      subj -> probs [N,S,2]
    """
    try:
        ...
        raise RuntimeError("scikit-learn is required for LDA distillation pretrain") from e

    out: dict[int, np.ndarray] = {}
    for subj, sw in items.items():
        X = np.asarray(sw.X, dtype=np.float32)  # [N,S,D]
        y = np.asarray(sw.y, dtype=np.int64)  # [N]
        _n_trials, n_steps, obs_dim = X.shape

        Xtr = X.reshape(-1, obs_dim)
        ytr = np.repeat(y, n_steps)
...
def _teacher_policy_from_probs(probs: np.ndarray, *, conf_power: float = 1.0) -> np.ndarray:
    """
    Build 3-action soft teacher policy pi from LDA probs [N,S,2]:
      conf = max(p_left, p_right)
      pi_continue = 1 - conf
      pi_commit_left = p_left * conf
      pi_commit_right = p_right * conf
    Returns:
      pi: [N,S,3] float32 rows sum to 1.
    """
    ...
    p_left = probs[..., 0]
    p_right = probs[..., 1]
    conf = np.maximum(p_left, p_right)
    conf_power_f = float(conf_power)
    if not (conf_power_f > 0.0):
        raise ValueError(f"conf_power must be > 0, got {conf_power}")
    if conf_power_f != 1.0:
        conf = np.power(conf, np.float32(conf_power_f)).astype(np.float32, copy=False)
    pi_continue = 1.0 - conf
    pi_left = p_left * conf
    pi_right = p_right * conf
    pi = np.stack([pi_continue, pi_left, pi_right], axis=-1).astype(np.float32, copy=False)
    # Numerical safety: renormalize.
    z = np.maximum(pi.sum(axis=-1, keepdims=True), 1e-12).astype(np.float32, copy=False)
    return (pi / z).astype(np.float32, copy=False)
...
def main() -> None:
    ...
    policy_distill = pre.get("policy_distill") or {}
    if not isinstance(policy_distill, dict):
        raise TypeError(f"pretrain.policy_distill must be a dict, got {type(policy_distill).__name__}")
    conf_power = float(policy_distill.get("conf_power", 1.0))
    ...
    for subj, sw in subj_items.items():
        ...
        probs = np.asarray(probs_by_subj[int(subj)], dtype=np.float32)
        pi = _teacher_policy_from_probs(probs, conf_power=conf_power)
    ...
    pi_all = np.concatenate(all_pi, axis=0)

    X_train, y_train, pi_train, X_val, y_val, pi_val = _split_train_val(
        X=X_all, y=y_all, pi=pi_all, val_frac=val_frac, seed=seed
    )
    ...
    logger = CSVLogger(out_dir / "metrics_policy_distill.csv")
    best_val_acc = -1.0
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, int(perm.shape[0]), batch_trials):
            ...
            pib = torch.as_tensor(pi_train[idx], device=device, dtype=torch.float32)  # [B,S,3]
            ...
            for t in range(S):
                ...
                loss = loss + -(pib[:, t] * logp).sum(dim=-1).mean()
            loss = loss / float(S)
        ...
        with torch.no_grad():
            ...
            for start in range(0, int(X_val.shape[0]), batch_trials):
                ...
                pib = torch.as_tensor(pi_val[start : start + batch_trials], device=device, dtype=torch.float32)
                ...
                for t in range(int(xb.shape[1])):
                    ...
                    commit_logits = logits[:, 1:3]
                    pred = torch.argmax(commit_logits, dim=-1)  # 0/1
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(ac.state_dict(), str(run_paths.ckpt_dir / "policy_distill_best.pt"))

    torch.save(ac.state_dict(), str(run_paths.ckpt_dir / "policy_distill_last.pt"))
    logger.close()
    print(f"[policy_distill] done -> {run_paths.out_dir}")
